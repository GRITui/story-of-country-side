...
- **Files used here** (unmodified from the mirror, renamed to nothing — original filenames kept):
  ...
  - `open_002.wav` (0.32s) — used for `"festival_start"` sfx (`FestivalManager.festival_started`)
  - `close_002.wav` (0.33s, deliberately paired with `open_002.wav` above for a symmetric start/end feel) — used for `"festival_end"` sfx (`FestivalManager.festival_ended`)
  - `confirmation_003.wav` (0.32s, a third distinct "confirmation" variant, not yet used by `"harvest"`/`"levelup"` above) — used for `"bundle_complete"` sfx (`CommunityGoalManager.bundle_completed`)
...
(`pluck`/`confirmation`/`bong`/`select`/`glass`/`maximize`/`open`/`close`
read as plausible fits for a coin pickup, a positive action confirmation, a
single notification chime, a bigger celebratory moment, a skill-up chime, a
quest-complete ding, a rising upgrade cue, and a festival opening/closing
respectively) plus measured duration/file-size via Python's `wave` module
(shorter, snappier clips for the frequent SFX; the longer variant within
each name-prefix family picked for the rarer/bigger events — e.g.
`select_006.wav` at 1.95s is a clear outlier among the pack's eight
`select_*` files, 001/002/007/008 are ~60ms clicks and 003-005 are ~385ms,
so 006 is very likely a distinct, more elaborate sound bundled under the
same prefix, not just another short click; same reasoning applied to
picking `confirmation_002.wav` over `_001` for the rarer level-up event,
`glass_004.wav` -- the longest of six `glass_*` files -- over the shorter
ones already spoken for, and `open_002.wav`/`close_002.wav` deliberately
paired together for festival start/end since they're near-identical
duration variants within their own prefix families). This is a reasonable
best-effort, not a verified-by-ear choice -- flagging for whoever next has
real audio playback (or asks a human) to swap any of these if actually
listening reveals a mismatch. Swapping is a one-line change: each is loaded
by `register_sfx_asset()` in `audio_manager.gd`'s
